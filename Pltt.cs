...
/* CHANGE LOG
 * [UPD] Palette OBS'd in favor of a ConvertToPalette overload
 * [UPD] IndexRotator renamed to just Rotator
 * [UPD] RotateColors doesn't internally Clone anymore
 * [ADD] Dispose
 * v2.3, 230716
 * [NEW] IndexRotator
 * v1.2, 160712
...
namespace Idmr.LfdReader
{
	...
	/// <para>The <see cref="Rotator"/> is a rarely used functionality that cycles the defined color range at a given frequency.
	/// The <see cref="Rotator.FrameDivider"/> is such that (0x1333 / <i>FrameDivider</i> = n), where the cycle increments every "nth" frame.
	/// The indexes wrap around, so a starting range of {1 2 3} goes to {2 3 1}, {3 1 2}, then back to {1 2 3}.
	/// The Index values define the range. Right now the only known use is in CITY.LFD, it's visible as the blinking red aerial indicators and the yellow twinkling window lights.</para></example>
	...
	public partial class Pltt : Resource
	{
		...
		readonly List<Rotator> _rotators = new List<Rotator>();
		RotatorIndexer _rotIndexer;
		...
		public Pltt()
		{
			...
			_rotIndexer = new RotatorIndexer(this);
		}
		...
		public Pltt(FileStream stream, long filePosition) : this()
		{
		...
		/// <param name="filePosition">The offset of the beginning of the resource.</param>
		/// <exception cref="Common.LoadFileException">Typically due to file corruption.</exception>
		public Pltt(string path, long filePosition) : this()
		{
		...
		public override void DecodeResource(byte[] raw, bool containsHeader)
		{
			...
			if (RotatorCount == 0) return;

			for (int i = 0; i < RotatorCount; i++)
			{
				_rotators.Add(new Rotator
				{
		...
		{
			byte[] raw = new byte[NumberOfColors * 3 + 3 + 4 * RotatorCount];
			raw[0] = _startIndex;
			raw[1] = _endIndex;
			int offset = 2;
		...
		/// <summary>Generates a ColorPalette of the resource.</summary>
		/// <returns>A ColorPalette to be applied to an image.</returns>
		/// <remarks>Unused colors are set to <see cref="Color.Transparent"/>.</remarks>
		public ColorPalette ConvertToPalette() => ConvertToPalette(new Pltt[] { this });
		...
		[Obsolete("Use ConvertToPalette()", true)]
		...
		/// <summary>Gets the number of <see cref="Rotator"/> entries defined in the resource.</summary>
		/// <remarks>This is almost always zero, but is used in CITY.LFD.</remarks>
		public byte RotatorCount { get; private set; }

		/// <summary>Gets the indexer for the rotators.</summary>
		public RotatorIndexer Rotators => _rotIndexer;
		...
		public struct Rotator
		{
			...
			{
				var old = RotatedColors[0];
				for (int c = 0; c < RotatedColors.Length - 1; c++)
					RotatedColors[c] = RotatedColors[c + 1];
				RotatedColors[RotatedColors.Length - 1] = old;
			}
        }
	}
}
